...
Настройка по умолчанию: **`packet_rate=standard`** (все пресеты, кроме Destitute).
...
* **`snapshot_buffer=auto`**: автоматически устанавливает наилучшее значение интерполяции для каждого класса. Для хитскан классов (Разведчик, Пулеметчик, Инженер и Снайпер) это значение `snapshot_buffer=x1`. Для классов со снарядами  (солдат, поджигатель, подрывник и медик) это значение `snapshot_buffer_off`. В настоящее время для шпиона это значение `snapshot_buffer_off`.
* **`snapshot_buffer=off`**: Самый быстрый вариант с минимальным временем буферизации для защиты (**15мс lerp** при использовании `packet_rate=standard`, или **30мс lerp** при использовании `packet_rate=congestion`).
* **`snapshot_buffer=x1`**: самый безопасный вариант с минимальной задержкой снимка при использовании каждого второго снимка (**30мс lerp** при использовании `packet_rate=standard` или **60мс lerp** при использовании `packet_rate=congestion`).
* **`snapshot_buffer=x2`**: Сильно защищает от потери пакетов, используя каждый третий снимок (**45мс lerp** при использовании `packet_rate=standard`, или **90мс lerp** при использовании `packet_rate=congestion` ).
...
* **`shading=low`**: Затенение выкл.
* **`shading=medium`**
* **`shading=high`**: Затенение вкл.
...
* **`flashlight=on`**: Включает поддержку фонарика. Если  модуль `shadows` имеет значение `off`, он будет установлен на уровень `low`.

### Эффекты

Управляет эффектами частиц, оружием эффектами и другими разными эффектами.
...
* **`effects=very_low`**: Отключает выброс гильзы, отключает дульные вспышки, отключает трассеры от первого лица, отключает брызги воды, отключает временное (шприцевое) столкновение. Объединяет порождающиеся частицы в существующие частицы (включая индикаторы пуль и пламя) вместе, отключает погодные частицы, снижает плотность / качество частиц, отключает дыры от пуль.
* **`effects=low`**: Отключает выброс гильзы, отключает дульные вспышки, отключает трассеры от первого лица, отключает брызги воды, отключает временное (шприцевое) столкновение. Отключены погодные частицы, уменьшена плотность / качество частиц, отключены пятна от пуль.
* **`effects=medium`**: Отключает выброс гильзы, отключает дульные вспышки, показывает индикаторы от первого лица, включает брызги воды, отключает временное (шприцевое) столкновение. Отключены погодные частицы, уменьшена плотность / качество частиц, отключены пятна от пуль.
* **`effects=high`**: Отключает выброс гильзы, отключает вспышки выстрела, показывает трассеры от первого лица и делает трассеры пули толще, позволяет разбрызгивать воду. Включает атмосферные частицы, использует полную плотность / качество частиц, разрешает образование пятен от пуль.
* **`effects=ultra`**: Включает выброс гильзы, включает дульные вспышки, показывает трассеры от первого лица и делает трассеры пули толще, позволяет разбрызгивать воду. Включает погодные частицы, использует полную плотность / качество частиц, включает новую систему эффектов удара пули, позволяет создавать пятна удара пули.
...
!!! Заметка
    В пресете Destitute декали принудительно невидимы.
...
* **`props=low`**: Отключает объекты на стороне клиента и листву, модели объектов по умолчанию.
...
Настройка по умолчанию: **`sheens_speed=slow`** (все пресеты, кроме Destitute).
...
Настройка по умолчанию: **`hud_contracts=custom`** (все пресеты, кроме Destitute).
...
Настройка по умолчанию: **`hud_panels=high`** (все пресеты, кроме Low и Destitute).
...
Настройка по умолчанию: **`match_hud=on`** (все пресеты, кроме Destitute).
...
Настройка по умолчанию: **`messages=on`** (все пресеты, кроме Destitute).
...
Настройка по умолчанию: **`killfeed=on`** (все пресеты, кроме Destitute).
...
Настройка по умолчанию: **`killstreaks=high`** (все пресеты, кроме Low и Destitute).
...
Настройка по умолчанию: **`hud_achievement=on`** (все пресеты).
...
Настройка по умолчанию: **`console=on`** (все пресеты, кроме Destitute).
...
Управляет живой картой, которая появляется на заднем плане главного меню.
...
!!! Заметка
    В Low и Destitute пресетах VPKs, пространственные и позиционные эффекты  (называемый [DSP](https://developer.valvesoftware.com/wiki/DSP) В движке Source) полностью отключен.
...
Настройка по умолчанию: **`voice_chat=on`** (все пресеты, кроме Destitute).
...
Настройки по умолчанию: **`party_mode=invite`** (все пресеты).
...
Настройки по умолчанию: **`logo=on`** (все пресеты, кроме Destitute).
